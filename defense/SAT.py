...
def get_tf_bn_mean_var(torch_name):
    ...
    if torch_name.startswith('block') and 'bn' in torch_name:
        ...
        if b == 1 and l == 0 and n == 1:
            ...
            m = "unit_{:d}_{:d}/residual_only_activation/BatchNorm/moving_mean:0".format(b, l)
            v = "unit_{:d}_{:d}/residual_only_activation/BatchNorm/moving_variance:0".format(b, l)
        elif n == 2:
...
class DefenseSAT(torch.nn.Module):
    meta_path = 'checkpoints/SAT/adv_trained/checkpoint-70000.meta'
    tf_ckpt = 'checkpoints/SAT/adv_trained/checkpoint-70000'
    pt_path = 'checkpoints/SAT/SAT.pt'

    def __init__(self, load_tf=False):
        super(DefenseSAT, self).__init__()
    ...
    def load_tf_weights(self):
        ...
        with tf.Session() as sess:
            ...
            # x = dg.get_tensor_by_name('input:0')
